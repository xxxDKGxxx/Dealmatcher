...
class _CreateOfferPageState extends State<CreateOfferPage> {
  ...
  void _submit() {
    if (_formKey.currentState!.validate()) {
      ...
      final data = {
        ...
        "images": _images.map((e) => e.path).toList(), // In future this would be base64 or upload paths
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Offer form is valid. Ready for API integration.')),
      );
    }
  }
  ...
  Future<List<Property>> _fetchProperties(int categoryId) async {
    ...
    if (categoryId == 0) { // Computers
      return [
        Property(id: 1, name: "RAM (GB)", type: PropertyType.NUMBER, options: []),
        Property(id: 2, name: "Storage (GB)", type: PropertyType.NUMBER, options: []),
        Property(id: 3, name: "OS", type: PropertyType.SELECT, options: ["Windows", "Linux", "MacOS"]),
        Property(id: 4, name: "Is New", type: PropertyType.BOOLEAN, options: []),
      ];
    } else if (categoryId == 1) { // Apartments
      return [
        Property(id: 5, name: "Rooms", type: PropertyType.NUMBER, options: []),
        Property(id: 6, name: "Floor", type: PropertyType.NUMBER, options: []),
        Property(id: 7, name: "Has Balcony", type: PropertyType.BOOLEAN, options: []),
        Property(id: 8, name: "Heating", type: PropertyType.SELECT, options: ["Gas", "Electric", "Central"]),
      ];
    }
    return [];
  }
                  ...
                  SliverList.list(
                    children: [
                      ...
                      FutureBuilder<List<Property>>(
                        ...
                        builder: (context, snapshot) {
                          if (_selectedCategory == null) {
                            return const Center(child: Text("Please select a category to see its properties"));
                          }
                          if (snapshot.connectionState == ConnectionState.waiting) {
                            return const Center(child: CircularProgressIndicator());
                          }
                          ...
                          if (!snapshot.hasData || snapshot.data!.isEmpty) {
                            return const Text("No specific properties for this category");
                          }
                          ...
                          return Column(
                            ...
                            children: [
                              ...
                              ...properties.map((prop) {
                                if (prop.type == PropertyType.NUMBER) {
                                  return Padding(
                                    ...
                                    child: TextFormField(
                                      ...
                                      onChanged: (value) {
                                        _properties[prop.name] = double.tryParse(value) ?? 0;
                                      },
                                      validator: (value) {
                                ...
                                } else if (prop.type == PropertyType.SELECT) {
                                  return Padding(
                                    padding: const EdgeInsets.only(bottom: 16.0),
                      ...
                      ElevatedButton(
                        ...
                        child: const Text("Create Offer", style: TextStyle(fontSize: 18)),
                      ),
