...
class AdminOfferActivityView extends StatefulWidget {
  const AdminOfferActivityView({super.key});

  @override
  State<StatefulWidget> createState() => _AdminOfferActivityViewState();
}

class _AdminOfferActivityViewState extends State<AdminOfferActivityView> {
  ...
  void _fetchActivity() {
    final offerIdText = _offerIdController.text.trim();
    if (offerIdText.isEmpty) {
      return;
    }

    final offerId = int.tryParse(offerIdText);
    if (offerId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter a valid Offer ID')),
      );
      return;
    }

    setState(() {
      _activityFuture = apiAdmin.getOfferActivity(offerId);
    });
  }
      ...
      children: [
        ...
        Expanded(
          child: _activityFuture == null
              ? const Center(
                  child: Text('Enter an Offer ID to view activities'),
                )
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (snapshot.hasData && snapshot.data != null) {
                      ...
                      if (activities.isEmpty) {
                        return const Center(
                          child: Text('No activities found for this offer.'),
                        );
                      }
